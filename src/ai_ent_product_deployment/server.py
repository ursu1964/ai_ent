...
class LocalProductServer:
    def __init__(self, config: LocalDeploymentConfig) -> None:
        ...
        self._server = ThreadingHTTPServer(
            ...
            _handler_factory(self.api, self.ui),
        )
...
def _handler_factory(api: ProductApiApp, ui: ProductUiApp) -> type[BaseHTTPRequestHandler]:
    class LocalProductHandler(BaseHTTPRequestHandler):
        ...
        def _dispatch(self, method: str) -> None:
            ...
            if parsed.path.startswith("/api/"):
                ...
                self._send(result.status_code, json.dumps(result.json(), sort_keys=True), "application/json")
                return
            if parsed.path.startswith("/ui/"):
                ...
                self._send(result.status_code, result.text(), result.content_type)
                return
            self._send(
                404,
